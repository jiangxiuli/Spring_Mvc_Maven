...
public class JvmInfo {
    ...
    private static void printGarbageCollectorInfo() {
        ...
        for (GarbageCollectorMXBean garbage : garbages) {
            System.out.println("垃圾收集器：名称=" + garbage.getName() + ",收集=" + garbage.getCollectionCount() + ",总花费时间=" + garbage.getCollectionTime() + ",内存区名称="
    ...
    private static void printMemoryInfo() {
        ...
        System.out.println("heap堆:");
        System.out.println("\t初始(M):" + heapMemory.getInit() / MB);
        System.out.println("\t最大(上限)(M):" + heapMemory.getMax() / MB);
        System.out.println("\t当前(已使用)(M):" + heapMemory.getUsed() / MB);
        System.out.println("\t提交的内存(已申请)(M):" + heapMemory.getCommitted() / MB);
        System.out.println("\t使用率:" + heapMemory.getUsed() * 100 / heapMemory.getCommitted() + "%");

        System.out.println("non-heap非堆:");
        MemoryUsage nonheapMemory = memory.getNonHeapMemoryUsage();
        System.out.println("\t初始(M):" + nonheapMemory.getInit() / MB);
        System.out.println("\t最大(上限)(M):" + nonheapMemory.getMax() / MB);
        System.out.println("\t当前(已使用)(M):" + nonheapMemory.getUsed() / MB);
        System.out.println("\t提交的内存(已申请)(M):" + nonheapMemory.getCommitted() / MB);
        System.out.println("\t使用率:" + nonheapMemory.getUsed() * 100 / nonheapMemory.getCommitted() + "%");
    }
    ...
    private static void printThreadInfo() {
        ...
        if (deadlockedIds != null && deadlockedIds.length > 0) {
            ...
            for (ThreadInfo deadlockInfo : deadlockInfos) {
                System.out.println("\t\t" + deadlockInfo.getThreadName() + "\t\t" + deadlockInfo.getThreadState() + "\t\t" + deadlockInfo.getBlockedTime() + "\t\t" + deadlockInfo.getWaitedTime()
