...
public class KaoyaResourceByLock {
    ...
    //创建生产者条件对象  
    private Condition pro_condition= resourceLock.newCondition();
    //创建消费者条件对象  
    private Condition con_condition= resourceLock.newCondition();
    /** 
     * 生产烤鸭 
     */  
    public  void product(String name){  
        ...
        try{  
            ...
            flag=true;//有烤鸭后改变标志  
            con_condition.signalAll();//通知消费线程可以消费了  
        }finally{  
            resourceLock.unlock();  
        }     
    }  
    ...
    public  void consume(){  
        ...
        try{  
        if(!flag){//如果没有烤鸭就等待  
            try{con_condition.await();}catch(InterruptedException e){}  
        }  
        System.out.println(Thread.currentThread().getName()+"...消费者........"+this.name);//消费烤鸭1  
        flag = false;  
        pro_condition.signalAll();//通知生产者生产烤鸭  
        }finally{  
            resourceLock.unlock();  
        }  
    }
